# SafeChat Funding Plan

**Prepared:** 2026-05-29
**Project:** SafeChat — International Chat Safety Protocol
**Applicant:** Rob Graham / FAMTEC (Fine Art Media Technology)
**Affiliation:** RMIT University, School of Design (PhD researcher)

---

## Executive Summary

SafeChat is source-available crisis safety infrastructure for AI chat systems. It detects distress signals locally (zero data collection), geo-locates users without GPS, and connects them to verified professional crisis resources across 34 countries. It is a routing layer, not a diagnostic tool.

This plan identifies tiered funding opportunities aligned with SafeChat's positioning as source-available digital public infrastructure for AI safety and suicide prevention.

---

## Tier 1 — Urgent (Apply Now)

### NLnet NGI Zero Commons Fund
- **Deadline:** June 1, 2026
- **Amount:** EUR 5,000–50,000
- **URL:** https://nlnet.nl/commonsfund/
- **Fit:** Strong match: privacy-preserving, local-first digital commons. License path should be clarified because BSL-1.1 is source-available, not OSI-approved. New calls every 2 months through 2027.
- **Action:** Submit application by June 1.

### Sovereign Tech Fund
- **Deadline:** Rolling (continuous)
- **Amount:** EUR 50,000+, no upper limit
- **URL:** https://www.sovereign.tech/programs/fund
- **Fit:** Source-available digital infrastructure; strongest if an OSI-approved licence path is clarified.
- **Note:** Requires OSI-approved license. BSL-1.1 is source-available, not OSI-approved. May need to discuss with STF or consider license path (BSL changes to MPL 2.0 on 2029-01-01).
- **Action:** Submit application, address license question upfront.

### Open Technology Fund — Internet Freedom Fund
- **Deadline:** Rolling (two-stage: concept note then full proposal)
- **Amount:** $10,000–$900,000 (ideal range $50,000–$200,000)
- **URL:** https://www.opentech.fund/funds/internet-freedom-fund/
- **Fit:** Privacy-first, offline-capable, zero data collection. Strong alignment with internet freedom principles.
- **Action:** Submit concept note.

---

## Tier 2 — Near-Term (Apply by End of 2026)

### Shuttleworth Foundation Fellowship
- **Deadline:** November 1, 2026 (March 2027 intake)
- **Amount:** Salary-equivalent fellowship + $275,000/year project funding
- **URL:** https://www.shuttleworthfoundation.org/funding/fellowship-programme/
- **Fit:** Excellent public-interest social innovation fit. SafeChat is a strong Shuttleworth-style project One of the highest-value opportunities on this list.
- **Action:** Begin application preparation. Shuttleworth values demonstrated traction and open licensing commitment.

### CZI Essential Open Source Software for Science (EOSS)
- **Deadline:** October 18, 2026
- **Amount:** $50,000–$200,000/year for 2 years
- **URL:** https://chanzuckerberg.com/rfa/essential-open-source-software-for-science/
- **Fit:** Frame SafeChat as essential infrastructure for AI mental health research. Requires scientific use case.
- **Action:** Identify research partners who use or would use SafeChat in their work.

### Echoing Green Fellowship
- **Deadline:** Expected to open September 2026 (for 2027 cycle)
- **Amount:** $100,000 over 18 months
- **URL:** https://echoinggreen.org/fellowship/apply/
- **Fit:** Early-stage social entrepreneurs. SafeChat through FAMTEC as social enterprise.
- **Action:** Monitor for 2027 cycle opening.

### Coefficient Giving (Open Philanthropy) — Technical AI Safety
- **Deadline:** Rolling
- **Amount:** $200,000–$2M/year
- **URL:** https://coefficientgiving.org/funds/navigating-transformative-ai/
- **Fit:** Frame SafeChat as scalable oversight infrastructure for AI systems. Adjacent to core alignment research but with direct real-world impact.
- **Action:** Submit proposal.

---

## Tier 3 — Australian Government & Research (2026–2027 Cycles)

### MRFF Million Minds Mental Health Research Mission
- **Deadline:** Check GrantConnect for current round status
- **Amount:** Part of $18.83M pool (announced May 2026)
- **URL:** https://www.health.gov.au/our-work/mrff/grant-opportunities-calendar
- **Fit:** Translating research into practical mental health solutions. SafeChat's crisis routing directly serves this mission.
- **Action:** Monitor GrantConnect. May require institutional (RMIT) submission.

### ARC Linkage Projects 2027
- **Deadline:** Expected to open January 2027
- **Amount:** $50,000–$300,000/year for 2–5 years
- **URL:** https://www.arc.gov.au/funding-research/funding-schemes/linkage-program/linkage-projects
- **Fit:** Excellent for PhD-adjacent work. FAMTEC as Partner Organisation, RMIT as administering university, SafeChat as the research output.
- **Action:** Begin assembling now. Needs RMIT support and FAMTEC cash/in-kind contribution documented.

### Australian AI Safety Institute (AISI)
- **Deadline:** TBC — institute launching 2026 with $29.9M
- **URL:** https://www.industry.gov.au/publications/national-ai-plan
- **Fit:** SafeChat is exactly the kind of AI safety infrastructure AISI should support. Local-first, source-available, compliance-aware.
- **Action:** Monitor for grant program announcements. Register interest when programs open.

### Suicide Prevention Australia — Research Grants
- **Deadline:** June 23, 2026
- **URL:** https://www.suicidepreventionaust.org/research-grants/
- **Fit:** Direct alignment. Innovation Research Grants and Implementation Science Grants are both strong matches.
- **Action:** Apply immediately. Confirm whether RMIT must administer, request partner letters, and use `docs/funding-sprint-2026-06.md` as the working brief.

### VicHealth Grants
- **Deadline:** Check VicHealth website
- **Amount:** Up to $150,000 (from $4.7M pool)
- **URL:** https://www.vichealth.vic.gov.au/funding/
- **Fit:** Targets young Victorians (0–25). SafeChat's AI chat safety skews young. Victorian deployment angle.
- **Action:** Monitor for current rounds.

### National Industry PhD Program (Round 8)
- **Deadline:** Expected late 2026/early 2027
- **URL:** https://www.aea.gov.au
- **Fit:** PhD researcher + industry partner (FAMTEC). SafeChat as the PhD-industry project.
- **Action:** Prepare for Round 8 when announced.

---

## Tier 4 — Strategic Positioning (Ongoing)

### Digital Public Goods Alliance Registry
- **Deadline:** Rolling (no cost)
- **URL:** https://www.digitalpublicgoods.net/registry
- **Fit:** Register SafeChat as a Digital Public Good (SDG 3: Good Health and Well-being). Raises profile for UN-adjacent funding.
- **Action:** Submit registration. Requires an OSI-approved open-source licence, clear ownership, platform independence, privacy compliance, and Sustainable Development Goal relevance — SafeChat needs a dual-licence or accelerated MPL path before submission.

### Corporate AI Safety Partnerships
- **OpenAI AI and Mental Health Grant Program** — $5,000–$100,000. Closed for 2026; watch for 2027 cycle. Very strong fit.
- **Anthropic Economic Futures Program** — $10,000–$50,000. Research grants on AI's societal impacts.
- **Google.org Impact Challenge** — $1M–$3M. Requires nonprofit/academic institutional applicant.
- **Action:** Build relationships now. Present SafeChat at relevant conferences (ISEA2026 Dubai). Reach out to AI safety teams directly.

### Mozilla Fellowship 2027
- **Deadline:** Expected late 2026
- **URL:** https://www.mozillafoundation.org/en/what-we-do/grantmaking/fellowship/
- **Fit:** User privacy, AI accountability, open internet. SafeChat's privacy-first architecture is exactly Mozilla's mandate.
- **Action:** Monitor for 2027 nominations.

---

## Licensing Consideration

SafeChat uses BSL-1.1 (Business Source License), which is source-available but **not OSI-approved**. Some funders (Sovereign Tech Fund, NLnet, DPGA) prefer or require OSI-approved licenses.

**Options:**
1. **Apply as-is** — BSL-1.1 is functionally open for most use cases. Some funders accept it. Explain the change date (MPL 2.0 on 2029-01-01).
2. **Dual license** — Add an OSI-approved license (Apache 2.0 or MPL 2.0) alongside BSL-1.1 for the grant-funded portions.
3. **Accelerate change date** — Move the MPL 2.0 change date earlier (e.g., 2027) to demonstrate open-source commitment.

This is a strategic decision with commercial implications. Consider before applying to license-strict funders.

---

## Timeline

| Month | Action |
|-------|--------|
| **June 2026** | Submit Suicide Prevention Australia application by June 23. Follow up NLnet June 1 submission. Submit Sovereign Tech Fund. Submit OTF concept note. Register with DPGA if licensing is acceptable. |
| **July 2026** | Submit Coefficient Giving proposal. Begin Shuttleworth preparation. |
| **August 2026** | Monitor MRFF, SPA, VicHealth rounds. Present at conferences. |
| **September 2026** | Watch for Echoing Green 2027 opening. Continue Shuttleworth prep. |
| **October 2026** | Submit CZI EOSS (Oct 18). Submit Shuttleworth (Nov 1 prep). |
| **November 2026** | Submit Shuttleworth Fellowship. Monitor Mozilla 2027 fellowship. |
| **December 2026** | Watch for OpenAI Mental Health grants 2027. Prepare ARC Linkage 2027. |
| **January 2027** | Submit ARC Linkage Projects. National Industry PhD Round 8 if open. |

---

## Budget Outline (For Grant Applications)

| Category | Annual Estimate |
|----------|----------------|
| Lead developer (Rob Graham, 0.5 FTE) | $45,000 |
| Crisis data verification (contract, twice-monthly) | $8,000 |
| Clinical literature review (contract) | $6,000 |
| Infrastructure (CDN, hosting, CI/CD) | $2,000 |
| Testing and security audit | $4,000 |
| Community building (conferences, workshops) | $5,000 |
| Language expansion (translation, local partners) | $10,000 |
| Legal review (annual) | $3,000 |
| **Total** | **$83,000** |

Scale up or down depending on grant size. NLnet applications should target the EUR 20,000–50,000 range. ARC Linkage and Shuttleworth can support the full budget and beyond.

---

## Key Pitch Points (For All Applications)

1. **Problem:** AI companion chatbots are being used by millions, particularly young people. VERA-MH research (2026) found significant gaps in how major AI systems respond to suicidal ideation. New York's AI Companion Law and FTC inquiries signal regulatory urgency.

2. **Solution:** SafeChat is a routing layer — it detects textual distress signals and connects users to verified professional crisis resources. It does not diagnose, treat, or replace professional services.

3. **Differentiators:** Zero data collection (all local), works offline (PWA), 34 countries, 647 automated tests, Samaritans-compliant, BSL-1.1 licensed, drop-in integration (one script tag).

4. **Traction:** Live at rob-e-graham.github.io/safechat. 66 verified helplines. Configurable Shield class with 6 presets for different deployment contexts. Active CHANGELOG with public git history documenting every improvement.

5. **Sustainability:** FAMTEC commercial licensing for entities over $100K revenue. Source-available core remains free for personal, research, education, nonprofit, and small commercial use.

---

*This document is a living plan. Update as deadlines pass and new opportunities emerge.*
